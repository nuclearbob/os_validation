...
func aptTests(t *testing.T) {
	...
	Convey("Apt should be able to upgrade installed packages", t, func() {
		run_cmd_with_env("DEBIAN_FRONTEND=noninteractive", "apt-get", "-o", "Dpkg::Options::=\"--force-confold\"", "dist-upgrade", "-q", "-y", "--allow-change-held-packages", "--allow-downgrades")
	})
	Convey("Apt should be able to install a new package", t, func() {
		run_cmd_with_env("DEBIAN_FRONTEND=noninteractive", "apt-get", "-o", "Dpkg::Options::=\"--force-confold\"", "install", "fortune-mod", "-q", "-y", "--allow-change-held-packages", "--allow-downgrades")
	})
}
